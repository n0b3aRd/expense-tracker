<div>
    <div class="page-header mb-4">
        <div class="row align-items-center">
            <div class="col">
                ...
                <h2 class="page-title">Regular Incomes</h2>
            </div>
        ...
        <div class="">
            <table class="table table-vcenter card-table">
                ...
                @forelse($incomes as $income)
                    <tr>
                        <td>{{ $income->name ?? '' }}</td>
                        <td><span class="badge bg-blue-lt">{{ $income->incomeCategory?->name ?? '' }}</span></td>
                        <td class="text-end">{{ number_format($income->regular_amount, 2) }}</td>
                        <td>
                            <a href="#" class="text-primary" data-bs-toggle="modal" data-bs-target="#editModal" wire:click="edit({{ $income->id }})">
                            ...
                            <a href="#" class="text-danger ms-1" data-bs-toggle="modal" data-bs-target="#deleteModel" wire:click="setTarget({{ $income->id }})">
    ...
    @include('regular_income.edit_modal')
    @include('regular_income.create_modal')
</div>
@push('custom_scripts')
    <script>
        document.addEventListener('livewire:initialized', () => {
        @this.on('regular-income-saved', (event) => {
            $('#createModal .btn-close').trigger('click');
        });
        @this.on('regular-income-updated', (event) => {
