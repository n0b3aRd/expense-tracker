<div>
        ...
        <div class="">
            <table class="table table-vcenter card-table">
                ...
                @forelse($categories as $category)
                    <tr>
                        <td>{{ $category->name ?? '' }}</td>
                        <td>{{ number_format($category->budget_allocation, 2) ?? '' }}</td>
                        <td>
    ...
    @include('expense_category.edit_modal')
    @include('expense_category.create_modal')
</div>
@push('custom_scripts')
    <script>
        document.addEventListener('livewire:initialized', () => {
        @this.on('expense-category-saved', (event) => {
            $('#createModal .btn-close').trigger('click');
        });
        @this.on('expense-category-updated', (event) => {
